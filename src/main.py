...
def run_algorithm_and_record_time(algorithm_name, search_function, initial_state, goal_state, puzzle_id, results):
    ...
    print(f"Tempo para o {algorithm_name} rodar no 8-Puzzle {puzzle_id}: {elapsed_time:.4f} segundos")
    print(f"Estado final encontrado na profundiade: {depth}")
    print(f"Nos gerados: {nodes_generated}")
    print(f"Tamanho máximo da pilha: {max_queue_size}")
    print(f"Pico de memória usada: {peak / 1024:.2f} KB\n")  
...
def main():
    ...
    print("Resultados salvos em 8puzzle_results.xlsx")
