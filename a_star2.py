...
from utils import makeDescendants, print_puzzle, euclidean_distance_heuristic


def search(initial_state, goal_state):
    queue = deque(
        [(initial_state, 0, euclidean_distance_heuristic(initial_state, goal_state), 0)]
    )
    explored = set()
            ...
            if descendant_tuple not in explored and descendant not in queue:
                queue.append(
                    (
                        descendant,
                        depth + 1,
                        euclidean_distance_heuristic(descendant, goal_state),
